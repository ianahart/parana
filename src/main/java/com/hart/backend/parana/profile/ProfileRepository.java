...
public interface ProfileRepository extends JpaRepository<Profile, Long> {
    ...
    @Query(value = """
               ...
               p.aboutLesson AS aboutLesson, u.firstName AS firstName, u.fullName AS fullName, u.role AS role, u.id AS userId, p.id AS id, p.bio AS bio, p.city AS city, p.firstLessonFree AS firstLessonFree,
                ...
                p.yearsSnowboarding AS yearsSnowboarding, p.avatarUrl AS avatarUrl, p.createdAt AS createdAt
                ) FROM Profile p
                INNER JOIN p.user u
                WHERE p.id = :profileId
            """)
